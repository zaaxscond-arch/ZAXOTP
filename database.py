# database.py - Sistem database dengan kode akses (RAHASIA)
...
# ========================================
# KODE AKSES LEVEL - RAHASIA (TIDAK DITAMPILKAN)
# ========================================
ACCESS_CODES = {
    "XAZMODESAD": "owner",
    "BLINGBLING": "admin",
...
def activate_with_code(device_id, code):
    ...
    # Cek di database kode rahasia
    if code not in ACCESS_CODES:
        return False, "❌ KODE SALAH! Cek channel WhatsApp untuk info kode terbaru."
    ...
    # Pesan sukses tanpa menyebut kode
    level_names = {
        "owner": "OWNER",
        "admin": "ADMIN", 
        "premium": "PREMIUM"
    }
    return True, f"✅ AKTIVASI BERHASIL! Level: {level_names.get(level, level)} - Unlimited akses!"
...
def get_user_stats():
    ...
    trial = sum(1 for u in db.values() if u.get("level") in ["trial", None])
